...
bool is_feasible(struc_sol *sol);
...
void branch_and_bound(int *final_sol)
{
  ...
  // No fixed elements yet
  mySol.fixed = 0;

  int current_best = 0;

  // Get the value and bound
  frac_bound(&mySol,mySol.fixed);

  current_best = mySol.val;
  ...
  while(QueueSize){
    ...
    // Get max 
    struc_sol first = removeMax();

    frac_bound(&first,first.fixed);

    // Check if it is not greater
    if(first.bound < current_best) break;

    // Make the children	
    struc_sol solAppend1;
    struc_sol solAppend0;

    // Copy the already made part
    copy_array(&first.solution_vec[1], &solAppend1.solution_vec[1]);
    copy_array(&first.solution_vec[1], &solAppend0.solution_vec[1]);

    // Combine another fixed element only if it fits the capacity
    if(first.fixed + 1 > Nitems) break;
    solAppend1.fixed = first.fixed + 1;
    solAppend0.fixed = first.fixed + 1;

   
    solAppend1.solution_vec[solAppend1.fixed] = 1;
    solAppend0.solution_vec[solAppend0.fixed] = 0;

    if(is_feasible(&solAppend1))
    {
      frac_bound(&solAppend1, solAppend1.fixed);

      //  Assert that current_best is infact the best
      if(solAppend1.val > current_best)
      {
        current_best = solAppend1.val;
        copy_array(&solAppend1.solution_vec[1], &final_sol[1]);
      }

     // Insert child to the pq
     insert(solAppend1);
   }//if

    if(is_feasible(&solAppend0))
    {
    
      frac_bound(&solAppend0, solAppend0.fixed);

      //  Assert that current_best is infact the best
      if(solAppend0.val > current_best)
      {
        current_best = solAppend0.val;
        copy_array(&solAppend0.solution_vec[1], &final_sol[1]);
      }//if
...
// Bool function to access members
bool is_feasible(struc_sol *sol)
{
  int currentWeight = 0;

...
bool find (Value_Type value, struct darray* arr)
{
  if(mode == LINEAR_SEARCH)
  {
    ...
    int leftmostestElemenet = 0;
    int rightmostestElement = arr->size - 1;
    int i = (int) (leftmostestElemenet + (rightmostestElement / 2 )) ;
    while(leftmostestElemenet <= rightmostestElement && leftmostestElemenet >= 0 
         && rightmostestElement < arr->size)
    {
      if(value == arr->cells[i])
       return true;
      else if(value < arr->cells[i])
        leftmostestElemenet++;
      else
        rightmostestElement++;
    }
